...
(define-library (raspberry-pi gpio pins)
  ...
  (export pin-mode! set-pull-up-down!
	  set-pin! set-pins! set-analog-pin!
	  read-pin read-analog-pin
  ...
  (begin
    ...
    (def-c (read-analog-pin "pin")
      "int reading = analogRead((int) (unbox_number(pin)));
       return_closcall1(data, k, obj_obj2int(reading));
      ")
    ...
    (def-c (set-analog-pin! "pin" "val")
      "analogWrite((int) (unbox_number(pin)), (int) (unbox_number(val)));
       return_closcall1(data, k, boolean_t);
      ")
